...
-- Translated     : 2026-07-15 15:37:42 CEST
...
FROM (
    SELECT
        ...
        MIN(CASE WHEN 4.0 * rn >= cnt THEN CAST(days_diff AS REAL) END) AS lq_days_first,
        MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(days_diff AS REAL) END) AS median_days_first,
        MIN(CASE WHEN 4.0 * rn >= 3 * cnt THEN CAST(days_diff AS REAL) END) AS uq_days_first
    FROM (
...
INNER JOIN (
    SELECT
        ...
        event_family,
        concept_id,
        MIN(CASE WHEN 4.0 * rn >= cnt THEN CAST(days_diff AS REAL) END) AS lq_days_closest,
        MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(days_diff AS REAL) END) AS median_days_closest,
        MIN(CASE WHEN 4.0 * rn >= 3 * cnt THEN CAST(days_diff AS REAL) END) AS uq_days_closest
    FROM (
        SELECT anchor_event, event_family, concept_id, days_diff,
            ROW_NUMBER() OVER (PARTITION BY anchor_event, event_family, concept_id ORDER BY days_diff) AS rn,
            COUNT(*)     OVER (PARTITION BY anchor_event, event_family, concept_id)                    AS cnt
...
SELECT
    ...
    MIN(CASE WHEN 20.0 * rn >= cnt       THEN CAST(days_diff AS REAL) END) AS p05_days,
    MIN(CASE WHEN 10.0 * rn >= cnt       THEN CAST(days_diff AS REAL) END) AS p10_days,
    MIN(CASE WHEN  5.0 * rn >= cnt       THEN CAST(days_diff AS REAL) END) AS p20_days,
    MIN(CASE WHEN  4.0 * rn >= cnt       THEN CAST(days_diff AS REAL) END) AS p25_days,
    MIN(CASE WHEN 10.0 * rn >= 3 * cnt  THEN CAST(days_diff AS REAL) END) AS p30_days,
    MIN(CASE WHEN  5.0 * rn >= 2 * cnt  THEN CAST(days_diff AS REAL) END) AS p40_days,
    MIN(CASE WHEN  2.0 * rn >= cnt       THEN CAST(days_diff AS REAL) END) AS p50_days,
    MIN(CASE WHEN  5.0 * rn >= 3 * cnt  THEN CAST(days_diff AS REAL) END) AS p60_days,
    MIN(CASE WHEN 10.0 * rn >= 7 * cnt  THEN CAST(days_diff AS REAL) END) AS p70_days,
    MIN(CASE WHEN  4.0 * rn >= 3 * cnt  THEN CAST(days_diff AS REAL) END) AS p75_days,
    MIN(CASE WHEN  5.0 * rn >= 4 * cnt  THEN CAST(days_diff AS REAL) END) AS p80_days,
    MIN(CASE WHEN 10.0 * rn >= 9 * cnt  THEN CAST(days_diff AS REAL) END) AS p90_days,
    MIN(CASE WHEN 20.0 * rn >= 19 * cnt THEN CAST(days_diff AS REAL) END) AS p95_days
FROM (
...
UNION ALL
SELECT CAST(YEAR(c.index_date) AS TEXT), DATEDIFF(DAY, c.index_date, dos.death_date)
FROM temp.cohort c
INNER JOIN temp.death_obs_status dos ON dos.person_id = c.person_id
WHERE dos.death_date >= c.index_date
;
...
SELECT
    prevalence_year,
    anchor_event,
    MIN(CASE WHEN 4.0 * rn >= cnt THEN CAST(days_to_death AS REAL) END) AS lq_days,
    MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(days_to_death AS REAL) END) AS median_days,
    MIN(CASE WHEN 4.0 * rn >= 3 * cnt THEN CAST(days_to_death AS REAL) END) AS uq_days
FROM (
...
UNION ALL
SELECT CAST(YEAR(c.index_date) AS TEXT), 'INDEX',
       DATEDIFF(DAY, c.index_date, MAX(op.observation_period_end_date))
FROM temp.cohort c
INNER JOIN @cdm_database_schema.observation_period op
  ON op.person_id = c.person_id
...
SELECT
    prevalence_year,
    anchor_event,
    MIN(CASE WHEN 4.0 * rn >= cnt THEN CAST(followup_days AS REAL) END) AS lq_followup_days,
    MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(followup_days AS REAL) END) AS median_followup_days,
    MIN(CASE WHEN 4.0 * rn >= 3 * cnt THEN CAST(followup_days AS REAL) END) AS uq_followup_days
FROM (
...
FROM (
    ...
    UNION ALL
    -- DX -> MET: by DX year
    SELECT 'DX_MET' AS pair, CAST(index_year_int AS TEXT) AS index_year, direction, COUNT(*) AS n_patients
    FROM dx_met_base
    ...
    UNION ALL
    -- DX -> L01: by DX year
    SELECT 'DX_L01' AS pair, CAST(index_year_int AS TEXT) AS index_year, direction, COUNT(*) AS n_patients
    FROM dx_l01_base
    GROUP BY index_year_int, direction
    UNION ALL
    -- MET -> L01: OVERALL
    ...
    UNION ALL
    -- MET -> L01: by MET year
    SELECT 'MET_L01' AS pair, CAST(index_year_int AS TEXT) AS index_year, direction, COUNT(*) AS n_patients
    FROM met_l01_base
    GROUP BY index_year_int, direction
) x
ORDER BY
...
JOIN (
    SELECT
        anchor_event,
        MIN(CASE WHEN 4.0 * rn >= cnt THEN CAST(age_years AS REAL) END) AS age_lq_years,
        MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(age_years AS REAL) END) AS age_median_years,
        MIN(CASE WHEN 4.0 * rn >= 3 * cnt THEN CAST(age_years AS REAL) END) AS age_uq_years
    FROM (
...
SELECT
    ...
    MIN(CASE WHEN cnt > @min_cell_count AND 10.0 * rn >= cnt      THEN CAST(gap_days AS REAL) END) AS p10_days,
    MIN(CASE WHEN cnt > @min_cell_count AND  4.0 * rn >= cnt      THEN CAST(gap_days AS REAL) END) AS p25_days,
    MIN(CASE WHEN cnt > @min_cell_count AND  2.0 * rn >= cnt      THEN CAST(gap_days AS REAL) END) AS p50_days,
    MIN(CASE WHEN cnt > @min_cell_count AND  4.0 * rn >= 3 * cnt  THEN CAST(gap_days AS REAL) END) AS p75_days,
    MIN(CASE WHEN cnt > @min_cell_count AND 10.0 * rn >= 9 * cnt  THEN CAST(gap_days AS REAL) END) AS p90_days
FROM (
...
FROM (
    SELECT
        ...
        MIN(CASE WHEN gap_death_after_obs IS NOT NULL AND  4.0 * rn >= non_null_cnt THEN CAST(gap_death_after_obs AS REAL) END) AS lq_gap_days,
        MIN(CASE WHEN gap_death_after_obs IS NOT NULL AND  2.0 * rn >= non_null_cnt THEN CAST(gap_death_after_obs AS REAL) END) AS median_gap_days,
        MIN(CASE WHEN gap_death_after_obs IS NOT NULL AND  4.0 * rn >= 3 * non_null_cnt THEN CAST(gap_death_after_obs AS REAL) END) AS uq_gap_days,
        MIN(CASE WHEN gap_death_after_obs IS NOT NULL AND 10.0 * rn >= 9 * non_null_cnt THEN CAST(gap_death_after_obs AS REAL) END) AS p90_gap_days
    FROM (
