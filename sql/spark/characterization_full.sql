...
CAST(NULL AS bigint) AS person_id,
	...
	CAST(NULL AS int) AS days_diff  WHERE 1 = 0;
WITH events  AS (SELECT person_id,  CAST('DX' as STRING) AS event_name, index_date AS event_date FROM x0brquscpatient_char
 UNION ALL
...
UNION ALL
SELECT CAST(YEAR(c.index_date) AS STRING), DATEDIFF(DAY, c.index_date, d.death_date)
FROM x0brqusccohort c
INNER JOIN (
 SELECT person_id, MIN(death_date) AS death_date
 FROM @cdm_database_schema.death
...
UNION ALL
SELECT CAST(YEAR(c.index_date) AS STRING), DATEDIFF(DAY, ms.first_met_date, d.death_date)
FROM x0brqusccohort c
INNER JOIN x0brquscmet_summary ms ON c.person_id = ms.person_id AND ms.first_met_date IS NOT NULL
INNER JOIN (
 SELECT person_id, MIN(death_date) AS death_date
...
CAST(NULL AS STRING) AS prevalence_year,
	...
	CAST(NULL AS int) AS n_deaths  WHERE 1 = 0;
...
CAST(NULL AS STRING) AS prevalence_year,
	CAST(NULL AS STRING) AS anchor_event,
	CAST(NULL AS int) AS n_deaths_in_dist,
	CAST(NULL AS DOUBLE) AS p05_days,
	CAST(NULL AS DOUBLE) AS p10_days,
	CAST(NULL AS DOUBLE) AS lq_days,
	CAST(NULL AS DOUBLE) AS median_days,
	CAST(NULL AS DOUBLE) AS uq_days,
	CAST(NULL AS DOUBLE) AS p90_days,
	CAST(NULL AS DOUBLE) AS p95_days  WHERE 1 = 0;
