...
SELECT
    ...
    MIN(CASE WHEN 10.0 * rn >= cnt      THEN CAST(gap_days AS NUMERIC) END) AS p10_days,
    MIN(CASE WHEN  4.0 * rn >= cnt      THEN CAST(gap_days AS NUMERIC) END) AS p25_days,
    MIN(CASE WHEN  2.0 * rn >= cnt      THEN CAST(gap_days AS NUMERIC) END) AS p50_days,
    MIN(CASE WHEN  4.0 * rn >= 3 * cnt THEN CAST(gap_days AS NUMERIC) END) AS p75_days,
    MIN(CASE WHEN 10.0 * rn >= 9 * cnt THEN CAST(gap_days AS NUMERIC) END) AS p90_days
FROM (
