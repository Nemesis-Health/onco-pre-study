...
-- Translated     : 2026-07-15 15:37:46 CEST
...
FROM (
    SELECT
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY (CAST(event_date AS DATE) - CAST(CAST(strptime('1900-01-01', '%Y%m%d') AS DATE) AS DATE)) ASC, event_date ASC
        ) AS rn
...
UNION ALL
SELECT CAST(YEAR(CAST(c.index_date AS DATE)) AS VARCHAR(4)), (CAST(dos.death_date AS DATE) - CAST(c.index_date AS DATE))
FROM cohort c
INNER JOIN death_obs_status dos ON dos.person_id = c.person_id
WHERE dos.death_date >= c.index_date
;
...
UNION ALL
SELECT CAST(YEAR(CAST(ms.first_met_date AS DATE)) AS VARCHAR(4)), (CAST(dos.death_date AS DATE) - CAST(ms.first_met_date AS DATE))
FROM cohort c
INNER JOIN met_summary ms ON c.person_id = ms.person_id AND ms.first_met_date IS NOT NULL
INNER JOIN death_obs_status dos ON dos.person_id = c.person_id
WHERE dos.death_date >= ms.first_met_date
...
SELECT
    CASE
        WHEN GROUPING(YEAR(CAST(c.index_date AS DATE))) = 1 THEN 'OVERALL'
        ELSE CAST(YEAR(CAST(c.index_date AS DATE)) AS VARCHAR(4))
    END,
    'INDEX',
    COUNT(*),
...
GROUP BY GROUPING SETS ((), (YEAR(CAST(c.index_date AS DATE))))
;
INSERT INTO death_stratum_counts (prevalence_year, anchor_event, n_patients, n_deaths, n_deaths_in_obs, n_deaths_out_obs)
SELECT
    CASE
        WHEN GROUPING(YEAR(CAST(ms.first_met_date AS DATE))) = 1 THEN 'OVERALL'
        ELSE CAST(YEAR(CAST(ms.first_met_date AS DATE)) AS VARCHAR(4))
    END,
    'FIRST_MET',
    COUNT(*),
...
INNER JOIN met_summary ms ON c.person_id = ms.person_id AND ms.first_met_date IS NOT NULL
LEFT JOIN death_obs_status dos ON dos.person_id = c.person_id
GROUP BY GROUPING SETS ((), (YEAR(CAST(ms.first_met_date AS DATE))))
;
...
UNION ALL
SELECT CAST(YEAR(CAST(c.index_date AS DATE)) AS VARCHAR(4)), 'INDEX',
       (CAST(MAX(op.observation_period_end_date) AS DATE) - CAST(c.index_date AS DATE))
FROM cohort c
INNER JOIN @cdm_database_schema.observation_period op
  ON op.person_id = c.person_id
 AND op.observation_period_end_date >= c.index_date
GROUP BY c.person_id, c.index_date, YEAR(CAST(c.index_date AS DATE))
UNION ALL
...
UNION ALL
SELECT CAST(YEAR(CAST(ms.first_met_date AS DATE)) AS VARCHAR(4)), 'FIRST_MET',
       (CAST(MAX(op.observation_period_end_date) AS DATE) - CAST(ms.first_met_date AS DATE))
FROM cohort c
INNER JOIN met_summary ms ON c.person_id = ms.person_id AND ms.first_met_date IS NOT NULL
INNER JOIN @cdm_database_schema.observation_period op
  ON op.person_id = c.person_id
 AND op.observation_period_end_date >= ms.first_met_date
GROUP BY c.person_id, ms.first_met_date, YEAR(CAST(ms.first_met_date AS DATE))
;
...
WITH base  AS (SELECT CASE
            WHEN GROUPING(YEAR(CAST(index_date AS DATE))) = 1 THEN  CAST('OVERALL' as TEXT) ELSE CAST(YEAR(CAST(index_date AS DATE)) AS VARCHAR(4))
...
met_l01_base AS (
    SELECT
        YEAR(CAST(first_met_date AS DATE)) AS index_year_int,
        CASE
            WHEN first_l01_date IS NULL  THEN 'NO_EVENT'
...
FROM (
    ...
    FROM (
        SELECT p.from_event, p.to_event, p.days_diff,
            CASE WHEN p.from_event = 'MET' THEN YEAR(CAST(ms.first_met_date AS DATE)) ELSE YEAR(CAST(pc.index_date AS DATE)) END AS index_year_int,
            ROW_NUMBER() OVER (PARTITION BY CASE WHEN p.from_event = 'MET' THEN YEAR(CAST(ms.first_met_date AS DATE)) ELSE YEAR(CAST(pc.index_date AS DATE)) END, p.from_event, p.to_event ORDER BY p.days_diff) AS rn,
            COUNT(*)     OVER (PARTITION BY CASE WHEN p.from_event = 'MET' THEN YEAR(CAST(ms.first_met_date AS DATE)) ELSE YEAR(CAST(pc.index_date AS DATE)) END, p.from_event, p.to_event)                    AS cnt
...
med_before AS (
    SELECT
        ...
        event_family,
        concept_id,
        MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(days_before AS NUMERIC) END) AS median_days_before
    FROM (
        SELECT
            anchor_event,
            event_family,
...
med_after AS (
    SELECT
        ...
        event_family,
        concept_id,
        MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(days_after AS NUMERIC) END) AS median_days_after
    FROM (
        SELECT
            anchor_event,
            event_family,
...
window_l01 AS (
    SELECT
        ...
        MAX(
            CASE
                WHEN le.event_date >= (wb.anchor_date + TO_DAYS(CAST(30 * wb.window_index AS INTEGER)))
                 AND le.event_date <  (wb.anchor_date + TO_DAYS(CAST(30 * (wb.window_index + 1) AS INTEGER)))
                THEN 1 ELSE 0
            END
...
window_denom AS (
    SELECT
        ...
        MAX(
            CASE
                WHEN op.observation_period_start_date <= (wb.anchor_date + TO_DAYS(CAST(30 * wb.window_index + 15 AS INTEGER)))
                 AND op.observation_period_end_date   >= (wb.anchor_date + TO_DAYS(CAST(30 * wb.window_index + 15 AS INTEGER)))
                THEN 1 ELSE 0
            END
...
SELECT
    ...
    MIN(CASE WHEN cnt > @min_cell_count AND  4.0 * rn >= 3 * cnt  THEN CAST(gap_days AS NUMERIC) END) AS p75_days,
    MIN(CASE WHEN cnt > @min_cell_count AND 10.0 * rn >= 9 * cnt  THEN CAST(gap_days AS NUMERIC) END) AS p90_days
FROM (
...
FROM (
    SELECT
        ...
        MIN(CASE WHEN gap_death_after_obs IS NOT NULL AND  4.0 * rn >= non_null_cnt THEN CAST(gap_death_after_obs AS NUMERIC) END) AS lq_gap_days,
        MIN(CASE WHEN gap_death_after_obs IS NOT NULL AND  2.0 * rn >= non_null_cnt THEN CAST(gap_death_after_obs AS NUMERIC) END) AS median_gap_days,
        MIN(CASE WHEN gap_death_after_obs IS NOT NULL AND  4.0 * rn >= 3 * non_null_cnt THEN CAST(gap_death_after_obs AS NUMERIC) END) AS uq_gap_days,
        MIN(CASE WHEN gap_death_after_obs IS NOT NULL AND 10.0 * rn >= 9 * non_null_cnt THEN CAST(gap_death_after_obs AS NUMERIC) END) AS p90_gap_days
    FROM (
...
agg AS (
    SELECT
        ...
        MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(obs_days AS NUMERIC) END) AS median_days
    FROM ranked
...
metrics AS (
    ...
    SELECT
        ...
        CAST(NULL AS NUMERIC) AS median_days
    ...
    SELECT
        ...
        MIN(CASE WHEN 2.0 * rn >= non_null_cnt
                 THEN CAST(days_past_death AS NUMERIC) END)
...
WITH family_counts  AS (SELECT  CAST('DX' as TEXT) AS event_family, person_id, n_dx_records  AS n_records FROM dx_summary
    UNION ALL
...
med AS (
    SELECT MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(days_before AS NUMERIC) END) AS median_days
    FROM (
        SELECT
            days_before,
...
med AS (
    SELECT MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(signed_days AS NUMERIC) END) AS median_days
    FROM (
        SELECT
            signed_days,
