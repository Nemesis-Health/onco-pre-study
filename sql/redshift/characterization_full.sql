...
CREATE TABLE #death_obs_status  ( person_id BIGINT,
    death_date DATE,
...
CREATE TABLE #followup_long  (prevalence_year VARCHAR(20),
    anchor_event VARCHAR(20),
    followup_days INT
)
DISTSTYLE ALL;
...
CREATE TABLE #followup_quantiles  (prevalence_year VARCHAR(20),
    anchor_event VARCHAR(20),
...
CREATE TABLE #l01_event_days  ( person_id BIGINT,
    event_day  DATE
)
DISTKEY(person_id);
...
CREATE TABLE #l01_consecutive_gaps  ( person_id BIGINT,
    subgroup   VARCHAR(10),
    gap_days   INT
)
DISTKEY(person_id);
...
window_l01 AS (
    SELECT
        ...
        MAX(
            CASE
                WHEN le.event_date >= DATEADD(DAY,CAST(30 * wb.window_index as int),wb.anchor_date)
                 AND le.event_date <  DATEADD(DAY,CAST(30 * (wb.window_index + 1) as int),wb.anchor_date)
                THEN 1 ELSE 0
            END
...
window_denom AS (
    SELECT
        ...
        MAX(
            CASE
                WHEN op.observation_period_start_date <= DATEADD(DAY,CAST(30 * wb.window_index + 15 as int),wb.anchor_date)
                 AND op.observation_period_end_date   >= DATEADD(DAY,CAST(30 * wb.window_index + 15 as int),wb.anchor_date)
                THEN 1 ELSE 0
            END
