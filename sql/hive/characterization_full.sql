...
-- Target dialect : hive
-- Translated     : 2026-07-15 15:37:38 CEST
...
CREATE TEMPORARY TABLE IF NOT EXISTS dtp_ancestor_concepts  (ancestor_concept_id BIGINT
);
-- EDIT THIS LIST
...
CREATE TEMPORARY TABLE IF NOT EXISTS dtp_concepts  (concept_id      BIGINT,
...
WITH events AS (
    SELECT 'INDEX' AS anchor_event, 'ODX' AS event_family, e.person_id, e.concept_id,
        day(CAST(e.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) AS days_from_anchor
    FROM other_dx_events e
    JOIN cohort c ON e.person_id = c.person_id
    UNION ALL
    ...
    SELECT 'FIRST_MET' AS anchor_event, 'ODX' AS event_family, e.person_id, e.concept_id,
        day(CAST(e.event_date AS TIMESTAMP) - CAST(ms.first_met_date AS TIMESTAMP)) AS days_from_anchor
    FROM other_dx_events e
    JOIN cohort c ON e.person_id = c.person_id
...
WITH obs_around_anchor AS (
    ...
    SELECT
        'INDEX' AS anchor_event,
        c.person_id,
        day(CAST(c.index_date AS TIMESTAMP) - CAST(op.observation_period_start_date AS TIMESTAMP)) AS lookback_days,
        day(CAST(op.observation_period_end_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP))   AS followup_days
    FROM cohort c
    INNER JOIN @cdm_database_schema.observation_period op
        ON  op.person_id = c.person_id
    ...
    SELECT
        'FIRST_MET' AS anchor_event,
        c.person_id,
        day(CAST(ms.first_met_date AS TIMESTAMP) - CAST(op.observation_period_start_date AS TIMESTAMP)) AS lookback_days,
        day(CAST(op.observation_period_end_date AS TIMESTAMP) - CAST(ms.first_met_date AS TIMESTAMP))   AS followup_days
    FROM cohort c
...
decedent_anchor AS (
    SELECT
        ...
        CASE WHEN po.last_obs_end > dos.death_date
             THEN day(CAST(po.last_obs_end AS TIMESTAMP) - CAST(dos.death_date AS TIMESTAMP)) END  AS days_past_death
    FROM cohort c
    INNER JOIN death_obs_status dos ON dos.person_id = c.person_id
    LEFT JOIN patient_obs po ON po.person_id = c.person_id
    WHERE dos.death_date >= c.index_date
    ...
    SELECT
        ...
        CASE WHEN po.last_obs_end > dos.death_date
             THEN day(CAST(po.last_obs_end AS TIMESTAMP) - CAST(dos.death_date AS TIMESTAMP)) END
    FROM cohort c
    INNER JOIN met_summary ms ON ms.person_id = c.person_id AND ms.first_met_date IS NOT NULL
    INNER JOIN death_obs_status dos ON dos.person_id = c.person_id
    LEFT JOIN patient_obs po ON po.person_id = c.person_id
...
gap AS (
    ...
    SELECT
        ma.person_id,
        day(CAST(dx.first_dx_date AS TIMESTAMP) - CAST(ma.first_met_date AS TIMESTAMP)) AS gap_days
    FROM met_all ma
    JOIN dx_all dx
      ON dx.person_id = ma.person_id
    WHERE ma.first_met_date < dx.first_dx_date
...
pair AS (
    ...
    SELECT
        ma.person_id,
        day(CAST(la.event_date AS TIMESTAMP) - CAST(ma.first_met_date AS TIMESTAMP)) AS days_diff,
        la.event_date
    FROM met_all ma
    JOIN l01_all la
...
gap AS (
    ...
    SELECT
        ma.person_id,
        day(CAST(da.first_dtp_date AS TIMESTAMP) - CAST(ma.first_met_date AS TIMESTAMP)) AS gap_days
    FROM met_all ma
    JOIN dtp_all da
...
pairs AS (
    ...
    SELECT
        pd.person_id,
        pd.root_concept_id,
        day(CAST(ld.l01_date AS TIMESTAMP) - CAST(pd.procedure_date AS TIMESTAMP)) AS gap_days
    FROM proc_dates pd
    JOIN l01_dates ld
...
WITH gdx_flags AS (
    ...
    SELECT
        g.person_id,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) <  0 THEN 1 ELSE 0 END) AS has_before,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) =  0 THEN 1 ELSE 0 END) AS has_day0,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) >  0 THEN 1 ELSE 0 END) AS has_after_strict
...
WITH first_general AS (
    ...
    SELECT
        gs.person_id,
        day(CAST(gs.first_gen_cancer_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) AS signed_days
...
WITH patient_concept AS (
    ...
    SELECT
        g.concept_id,
        g.person_id,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) >= -30  AND day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) <= -1 THEN 1 ELSE 0 END) AS in_before_30d,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) >= -90  AND day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) <= -1 THEN 1 ELSE 0 END) AS in_before_90d,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) >= -180 AND day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) <= -1 THEN 1 ELSE 0 END) AS in_before_180d,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) >= -365 AND day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) <= -1 THEN 1 ELSE 0 END) AS in_before_365d,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) <  0 THEN 1 ELSE 0 END) AS in_ever_before,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) =  0 THEN 1 ELSE 0 END) AS in_day0,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) >= 1 AND day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) <= 30  THEN 1 ELSE 0 END) AS in_after_30d,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) >= 1 AND day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) <= 90  THEN 1 ELSE 0 END) AS in_after_90d,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) >= 1 AND day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) <= 180 THEN 1 ELSE 0 END) AS in_after_180d,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) >= 1 AND day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) <= 365 THEN 1 ELSE 0 END) AS in_after_365d,
        MAX(CASE WHEN day(CAST(g.event_date AS TIMESTAMP) - CAST(c.index_date AS TIMESTAMP)) >  0 THEN 1 ELSE 0 END) AS in_ever_after
    FROM gen_cancer_events g
    JOIN cohort c
      ON g.person_id = c.person_id
