...
-- Translated     : 2026-04-27 15:05:02 BST
...
UNION ALL
SELECT CAST(EXTRACT(YEAR FROM c.index_date) AS VARCHAR(4)), (CAST(d.death_date AS DATE) - CAST(c.index_date AS DATE))
FROM cohort c
INNER JOIN (
    SELECT person_id, MIN(death_date) AS death_date
    FROM @cdm_database_schema.death
...
UNION ALL
SELECT CAST(EXTRACT(YEAR FROM c.index_date) AS VARCHAR(4)), (CAST(d.death_date AS DATE) - CAST(ms.first_met_date AS DATE))
FROM cohort c
INNER JOIN met_summary ms ON c.person_id = ms.person_id AND ms.first_met_date IS NOT NULL
INNER JOIN (
    SELECT person_id, MIN(death_date) AS death_date
