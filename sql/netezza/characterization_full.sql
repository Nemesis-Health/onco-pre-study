...
-- Translated     : 2026-07-15 15:37:06 CEST
...
------------------------------------------------------------
DROP TABLE dx_anchor_include IF EXISTS;
...
;
DROP TABLE dx_anchor_exclude IF EXISTS;
...
INSERT INTO dx_anchor_exclude (concept_id, include_descendants) VALUES
    ...
    (4280897, 1),
    (4200889, 1);
DROP TABLE dx_anchor_concepts IF EXISTS;
...
------------------------------------------------------------
DROP TABLE gen_cancer_concepts IF EXISTS;
...
------------------------------------------------------------
DROP TABLE other_dx_ancestor_concepts IF EXISTS;
...
;
DROP TABLE other_dx_concepts IF EXISTS;
...
------------------------------------------------------------
DROP TABLE met_ancestor_concepts IF EXISTS;
...
VALUES
    ...
    (36769180)  -- Metastasis
;
DROP TABLE met_concepts IF EXISTS;
...
------------------------------------------------------------
DROP TABLE l01_ancestor_concepts IF EXISTS;
...
VALUES
    (21601387)
;
DROP TABLE l01_concepts IF EXISTS;
...
------------------------------------------------------------
DROP TABLE dtp_ancestor_concepts IF EXISTS;
...
VALUES
    ...
    (4061650)
;
DROP TABLE dtp_concepts IF EXISTS;
...
-- F) EVENT TABLES
------------------------------------------------------------
DROP TABLE dx_events IF EXISTS;
...
DROP TABLE anchor_person IF EXISTS;
...
FROM dx_events
;
DROP TABLE other_dx_events IF EXISTS;
...
JOIN other_dx_concepts d
  ON co.condition_concept_id = d.concept_id
;
DROP TABLE gen_cancer_events IF EXISTS;
...
;
DROP TABLE met_events IF EXISTS;
...
;
DROP TABLE l01_events IF EXISTS;
...
DROP TABLE l01_ingredient_events IF EXISTS;
...
DROP TABLE cohort_attrition IF EXISTS;
...
DROP TABLE dx_summary IF EXISTS;
...
;
DROP TABLE other_dx_summary IF EXISTS;
...
GROUP BY e.person_id
;
DROP TABLE gen_cancer_summary IF EXISTS;
...
GROUP BY e.person_id
;
DROP TABLE met_summary IF EXISTS;
...
GROUP BY e.person_id
;
DROP TABLE l01_summary IF EXISTS;
...
------------------------------------------------------------
DROP TABLE event_code_counts IF EXISTS;
...
GROUP BY concept_id
;
DROP TABLE event_code_counts_before_after IF EXISTS;
...
GROUP BY
    ...
    e.concept_id
;
DROP TABLE event_code_counts_before_after_first_met IF EXISTS;
...
GROUP BY
    ...
    e.concept_id
;
DROP TABLE event_code_all_events IF EXISTS;
...
WHERE ms.first_met_date IS NOT NULL
;
DROP TABLE event_code_patient_chosen_first IF EXISTS;
...
FROM (
    SELECT
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY (CAST(event_date AS DATE) - CAST(TO_DATE('1900-01-01', 'yyyymmdd') AS DATE)) ASC, event_date ASC
        ) AS rn
    FROM event_code_all_events
) x
WHERE rn = 1
;
DROP TABLE event_code_patient_chosen_closest IF EXISTS;
...
WHERE rn = 1
;
DROP TABLE event_code_timing_summary IF EXISTS;
CREATE TEMP TABLE event_code_timing_summary  (anchor_event VARCHAR(20),
    event_family VARCHAR(20),
    concept_id BIGINT,
    n_patients_with_code_timing INT,
    lq_days_first FLOAT(6),
    median_days_first FLOAT(6),
    uq_days_first FLOAT(6),
    lq_days_closest FLOAT(6),
    median_days_closest FLOAT(6),
    uq_days_closest FLOAT(6)
);
...
FROM (
    SELECT
        ...
        MIN(CASE WHEN 4.0 * rn >= cnt THEN CAST(days_diff AS FLOAT(6)) END) AS lq_days_first,
        MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(days_diff AS FLOAT(6)) END) AS median_days_first,
        MIN(CASE WHEN 4.0 * rn >= 3 * cnt THEN CAST(days_diff AS FLOAT(6)) END) AS uq_days_first
    FROM (
...
INNER JOIN (
    SELECT
        ...
        event_family,
        concept_id,
        MIN(CASE WHEN 4.0 * rn >= cnt THEN CAST(days_diff AS FLOAT(6)) END) AS lq_days_closest,
        MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(days_diff AS FLOAT(6)) END) AS median_days_closest,
        MIN(CASE WHEN 4.0 * rn >= 3 * cnt THEN CAST(days_diff AS FLOAT(6)) END) AS uq_days_closest
    FROM (
        SELECT anchor_event, event_family, concept_id, days_diff,
            ROW_NUMBER() OVER (PARTITION BY anchor_event, event_family, concept_id ORDER BY days_diff) AS rn,
            COUNT(*)     OVER (PARTITION BY anchor_event, event_family, concept_id)                    AS cnt
...
;
DROP TABLE event_code_ba_events IF EXISTS;
...
FROM event_code_all_events
;
DROP TABLE event_code_patient_chosen_before_after_first IF EXISTS;
...
WHERE rn = 1
;
DROP TABLE event_code_patient_chosen_before_after_closest IF EXISTS;
...
WHERE rn = 1
;
DROP TABLE event_code_timing_before_after_summary IF EXISTS;
...
------------------------------------------------------------
DROP TABLE patient_char IF EXISTS;
...
------------------------------------------------------------
DROP TABLE patient_timing_pairs IF EXISTS;
...
;
DROP TABLE timing_pair_summary IF EXISTS;
...
SELECT
    ...
    MIN(CASE WHEN 20.0 * rn >= cnt       THEN CAST(days_diff AS FLOAT(6)) END) AS p05_days,
    MIN(CASE WHEN 10.0 * rn >= cnt       THEN CAST(days_diff AS FLOAT(6)) END) AS p10_days,
    MIN(CASE WHEN  5.0 * rn >= cnt       THEN CAST(days_diff AS FLOAT(6)) END) AS p20_days,
    MIN(CASE WHEN  4.0 * rn >= cnt       THEN CAST(days_diff AS FLOAT(6)) END) AS p25_days,
    MIN(CASE WHEN 10.0 * rn >= 3 * cnt  THEN CAST(days_diff AS FLOAT(6)) END) AS p30_days,
    MIN(CASE WHEN  5.0 * rn >= 2 * cnt  THEN CAST(days_diff AS FLOAT(6)) END) AS p40_days,
    MIN(CASE WHEN  2.0 * rn >= cnt       THEN CAST(days_diff AS FLOAT(6)) END) AS p50_days,
    MIN(CASE WHEN  5.0 * rn >= 3 * cnt  THEN CAST(days_diff AS FLOAT(6)) END) AS p60_days,
    MIN(CASE WHEN 10.0 * rn >= 7 * cnt  THEN CAST(days_diff AS FLOAT(6)) END) AS p70_days,
    MIN(CASE WHEN  4.0 * rn >= 3 * cnt  THEN CAST(days_diff AS FLOAT(6)) END) AS p75_days,
    MIN(CASE WHEN  5.0 * rn >= 4 * cnt  THEN CAST(days_diff AS FLOAT(6)) END) AS p80_days,
    MIN(CASE WHEN 10.0 * rn >= 9 * cnt  THEN CAST(days_diff AS FLOAT(6)) END) AS p90_days,
    MIN(CASE WHEN 20.0 * rn >= 19 * cnt THEN CAST(days_diff AS FLOAT(6)) END) AS p95_days
FROM (
...
;
DROP TABLE all_events_for_pairs IF EXISTS;
...
;
DROP TABLE first_event_dates IF EXISTS;
...
;
DROP TABLE patient_timing_pairs_first_to_closest IF EXISTS;
...
WHERE rn = 1
;
DROP TABLE timing_pair_summary_first_to_closest IF EXISTS;
...
GROUP BY from_event, to_event
;
DROP TABLE patient_timing_pairs_first_to_closest_before IF EXISTS;
...
WHERE rn = 1
;
DROP TABLE timing_pair_summary_first_to_closest_before IF EXISTS;
...
GROUP BY from_event, to_event
;
DROP TABLE patient_timing_pairs_first_to_closest_after IF EXISTS;
...
WHERE rn = 1
;
DROP TABLE timing_pair_summary_first_to_closest_after IF EXISTS;
...
GROUP BY from_event, to_event
;
DROP TABLE event_presence IF EXISTS;
...
DROP TABLE death_obs_status IF EXISTS;
...
;
DROP TABLE death_index_long IF EXISTS;
...
UNION ALL
SELECT CAST(DATE_PART('YEAR', c.index_date) AS VARCHAR(4)), (CAST(dos.death_date AS DATE) - CAST(c.index_date AS DATE))
FROM cohort c
INNER JOIN death_obs_status dos ON dos.person_id = c.person_id
WHERE dos.death_date >= c.index_date
;
DROP TABLE death_first_met_long IF EXISTS;
...
UNION ALL
SELECT CAST(DATE_PART('YEAR', ms.first_met_date) AS VARCHAR(4)), (CAST(dos.death_date AS DATE) - CAST(ms.first_met_date AS DATE))
FROM cohort c
INNER JOIN met_summary ms ON c.person_id = ms.person_id AND ms.first_met_date IS NOT NULL
INNER JOIN death_obs_status dos ON dos.person_id = c.person_id
WHERE dos.death_date >= ms.first_met_date
;
DROP TABLE death_stratum_counts IF EXISTS;
...
SELECT
    CASE
        WHEN GROUPING(DATE_PART('YEAR', c.index_date)) = 1 THEN 'OVERALL'
        ELSE CAST(DATE_PART('YEAR', c.index_date) AS VARCHAR(4))
    END,
    'INDEX',
    COUNT(*),
...
GROUP BY GROUPING SETS ((), (DATE_PART('YEAR', c.index_date)))
;
INSERT INTO death_stratum_counts (prevalence_year, anchor_event, n_patients, n_deaths, n_deaths_in_obs, n_deaths_out_obs)
SELECT
    CASE
        WHEN GROUPING(DATE_PART('YEAR', ms.first_met_date)) = 1 THEN 'OVERALL'
        ELSE CAST(DATE_PART('YEAR', ms.first_met_date) AS VARCHAR(4))
    END,
    'FIRST_MET',
    COUNT(*),
...
INNER JOIN met_summary ms ON c.person_id = ms.person_id AND ms.first_met_date IS NOT NULL
LEFT JOIN death_obs_status dos ON dos.person_id = c.person_id
GROUP BY GROUPING SETS ((), (DATE_PART('YEAR', ms.first_met_date)))
;
DROP TABLE death_timing_long IF EXISTS;
...
;
DROP TABLE death_timing_quantiles IF EXISTS;
...
SELECT
    prevalence_year,
    anchor_event,
    MIN(CASE WHEN 4.0 * rn >= cnt THEN CAST(days_to_death AS FLOAT(6)) END) AS lq_days,
    MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(days_to_death AS FLOAT(6)) END) AS median_days,
    MIN(CASE WHEN 4.0 * rn >= 3 * cnt THEN CAST(days_to_death AS FLOAT(6)) END) AS uq_days
FROM (
...
DROP TABLE followup_long IF EXISTS;
...
UNION ALL
SELECT CAST(DATE_PART('YEAR', c.index_date) AS VARCHAR(4)), 'INDEX',
       (CAST(MAX(op.observation_period_end_date) AS DATE) - CAST(c.index_date AS DATE))
FROM cohort c
INNER JOIN @cdm_database_schema.observation_period op
  ON op.person_id = c.person_id
 AND op.observation_period_end_date >= c.index_date
GROUP BY c.person_id, c.index_date, DATE_PART('YEAR', c.index_date)
UNION ALL
...
UNION ALL
SELECT CAST(DATE_PART('YEAR', ms.first_met_date) AS VARCHAR(4)), 'FIRST_MET',
       (CAST(MAX(op.observation_period_end_date) AS DATE) - CAST(ms.first_met_date AS DATE))
FROM cohort c
INNER JOIN met_summary ms ON c.person_id = ms.person_id AND ms.first_met_date IS NOT NULL
INNER JOIN @cdm_database_schema.observation_period op
  ON op.person_id = c.person_id
 AND op.observation_period_end_date >= ms.first_met_date
GROUP BY c.person_id, ms.first_met_date, DATE_PART('YEAR', ms.first_met_date)
;
DROP TABLE followup_quantiles IF EXISTS;
CREATE TEMP TABLE followup_quantiles  (prevalence_year VARCHAR(20),
    anchor_event VARCHAR(20),
    lq_followup_days FLOAT(6),
    median_followup_days FLOAT(6),
    uq_followup_days FLOAT(6)
);
...
SELECT
    prevalence_year,
    anchor_event,
    MIN(CASE WHEN 4.0 * rn >= cnt THEN CAST(followup_days AS FLOAT(6)) END) AS lq_followup_days,
    MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(followup_days AS FLOAT(6)) END) AS median_followup_days,
    MIN(CASE WHEN 4.0 * rn >= 3 * cnt THEN CAST(followup_days AS FLOAT(6)) END) AS uq_followup_days
FROM (
...
DROP TABLE l01_event_days IF EXISTS;
...
DROP TABLE l01_consecutive_gaps IF EXISTS;
...
WITH dx_met_base AS (
    SELECT
        DATE_PART('YEAR', index_date) AS index_year_int,
        CASE
...
met_l01_base AS (
    SELECT
        DATE_PART('YEAR', first_met_date) AS index_year_int,
        CASE
            WHEN first_l01_date IS NULL  THEN 'NO_EVENT'
...
ORDER BY
    ...
    CASE WHEN x.index_year = 'OVERALL' THEN NULL ELSE CAST(x.index_year  AS INTEGER) END,
    CASE x.direction
...
FROM (
    ...
    FROM (
        SELECT p.from_event, p.to_event, p.days_diff,
            CASE WHEN p.from_event = 'MET' THEN DATE_PART('YEAR', ms.first_met_date) ELSE DATE_PART('YEAR', pc.index_date) END AS index_year_int,
            ROW_NUMBER() OVER (PARTITION BY CASE WHEN p.from_event = 'MET' THEN DATE_PART('YEAR', ms.first_met_date) ELSE DATE_PART('YEAR', pc.index_date) END, p.from_event, p.to_event ORDER BY p.days_diff) AS rn,
            COUNT(*)     OVER (PARTITION BY CASE WHEN p.from_event = 'MET' THEN DATE_PART('YEAR', ms.first_met_date) ELSE DATE_PART('YEAR', pc.index_date) END, p.from_event, p.to_event)                    AS cnt
...
WITH events AS (
    SELECT 'INDEX' AS anchor_event, 'ODX' AS event_family, e.person_id, e.concept_id,
        (CAST(e.event_date AS DATE) - CAST(c.index_date AS DATE)) AS days_from_anchor
    FROM other_dx_events e
    JOIN cohort c ON e.person_id = c.person_id
    UNION ALL
    ...
    SELECT 'FIRST_MET' AS anchor_event, 'ODX' AS event_family, e.person_id, e.concept_id,
        (CAST(e.event_date AS DATE) - CAST(ms.first_met_date AS DATE)) AS days_from_anchor
    FROM other_dx_events e
    JOIN cohort c ON e.person_id = c.person_id
...
med_before AS (
    SELECT
        ...
        event_family,
        concept_id,
        MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(days_before AS FLOAT(6)) END) AS median_days_before
    FROM (
        SELECT
            anchor_event,
            event_family,
...
med_after AS (
    SELECT
        ...
        event_family,
        concept_id,
        MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(days_after AS FLOAT(6)) END) AS median_days_after
    FROM (
        SELECT
            anchor_event,
            event_family,
...
window_l01 AS (
    SELECT
        ...
        MAX(
            CASE
                WHEN le.event_date >= (wb.anchor_date + 30 * wb.window_index)
                 AND le.event_date <  (wb.anchor_date + 30 * (wb.window_index + 1))
                THEN 1 ELSE 0
            END
...
window_denom AS (
    SELECT
        ...
        MAX(
            CASE
                WHEN op.observation_period_start_date <= (wb.anchor_date + 30 * wb.window_index + 15)
                 AND op.observation_period_end_date   >= (wb.anchor_date + 30 * wb.window_index + 15)
                THEN 1 ELSE 0
            END
...
ORDER BY
    ...
    CASE WHEN s.prevalence_year = 'OVERALL' THEN NULL ELSE CAST(s.prevalence_year  AS INTEGER) END,
    ...
    SELECT
        ...
        CASE
            ...
            WHEN year_of_birth IS NOT NULL
                THEN (CAST(anchor_date AS DATE) - CAST(TO_DATE(TO_CHAR(year_of_birth,'0000')||'-'||TO_CHAR(7,'00')||'-'||TO_CHAR(1,'00'), 'YYYY-MM-DD') AS DATE)) / 365.25
            ELSE NULL
        END AS age_years
    FROM base
)
...
FROM (
    SELECT
        ...
        CAST(100.0 * SUM(CASE WHEN gender_concept_id = 8507 THEN 1 ELSE 0 END) / NULLIF(COUNT(*), 0) AS FLOAT(6)) AS pct_male,
        CAST(100.0 * SUM(CASE WHEN gender_concept_id = 8532 THEN 1 ELSE 0 END) / NULLIF(COUNT(*), 0) AS FLOAT(6)) AS pct_female
    FROM ages
...
JOIN (
    SELECT
        anchor_event,
        MIN(CASE WHEN 4.0 * rn >= cnt THEN CAST(age_years AS FLOAT(6)) END) AS age_lq_years,
        MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(age_years AS FLOAT(6)) END) AS age_median_years,
        MIN(CASE WHEN 4.0 * rn >= 3 * cnt THEN CAST(age_years AS FLOAT(6)) END) AS age_uq_years
    FROM (
...
SELECT
    ...
    MIN(CASE WHEN cnt > @min_cell_count AND 10.0 * rn >= cnt      THEN CAST(gap_days AS FLOAT(6)) END) AS p10_days,
    MIN(CASE WHEN cnt > @min_cell_count AND  4.0 * rn >= cnt      THEN CAST(gap_days AS FLOAT(6)) END) AS p25_days,
    MIN(CASE WHEN cnt > @min_cell_count AND  2.0 * rn >= cnt      THEN CAST(gap_days AS FLOAT(6)) END) AS p50_days,
    MIN(CASE WHEN cnt > @min_cell_count AND  4.0 * rn >= 3 * cnt  THEN CAST(gap_days AS FLOAT(6)) END) AS p75_days,
    MIN(CASE WHEN cnt > @min_cell_count AND 10.0 * rn >= 9 * cnt  THEN CAST(gap_days AS FLOAT(6)) END) AS p90_days
FROM (
    ...
    SELECT
        ...
        CASE
            WHEN dos.death_date > po.last_obs_end
                THEN (CAST(dos.death_date AS DATE) - CAST(po.last_obs_end AS DATE))
            ELSE NULL
...
WITH obs_around_anchor AS (
    ...
    SELECT
        'INDEX' AS anchor_event,
        c.person_id,
        (CAST(c.index_date AS DATE) - CAST(op.observation_period_start_date AS DATE)) AS lookback_days,
        (CAST(op.observation_period_end_date AS DATE) - CAST(c.index_date AS DATE))   AS followup_days
    FROM cohort c
    INNER JOIN @cdm_database_schema.observation_period op
        ON  op.person_id = c.person_id
    ...
    SELECT
        'FIRST_MET' AS anchor_event,
        c.person_id,
        (CAST(ms.first_met_date AS DATE) - CAST(op.observation_period_start_date AS DATE)) AS lookback_days,
        (CAST(op.observation_period_end_date AS DATE) - CAST(ms.first_met_date AS DATE))   AS followup_days
    FROM cohort c
    INNER JOIN met_summary ms
...
agg AS (
    SELECT
        ...
        MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(obs_days AS FLOAT(6)) END) AS median_days
    FROM ranked
...
decedent_anchor AS (
    SELECT
        ...
        CASE WHEN po.last_obs_end > dos.death_date
             THEN (CAST(po.last_obs_end AS DATE) - CAST(dos.death_date AS DATE)) END  AS days_past_death
    FROM cohort c
    INNER JOIN death_obs_status dos ON dos.person_id = c.person_id
    LEFT JOIN patient_obs po ON po.person_id = c.person_id
    WHERE dos.death_date >= c.index_date
    ...
    SELECT
        ...
        CASE WHEN po.last_obs_end > dos.death_date
             THEN (CAST(po.last_obs_end AS DATE) - CAST(dos.death_date AS DATE)) END
    FROM cohort c
    INNER JOIN met_summary ms ON ms.person_id = c.person_id AND ms.first_met_date IS NOT NULL
    INNER JOIN death_obs_status dos ON dos.person_id = c.person_id
    LEFT JOIN patient_obs po ON po.person_id = c.person_id
...
metrics AS (
    ...
    SELECT
        ...
        CAST(NULL AS FLOAT(6)) AS median_days
    ...
    SELECT
        ...
        MIN(CASE WHEN 2.0 * rn >= non_null_cnt
                 THEN CAST(days_past_death AS FLOAT(6)) END)
...
med AS (
    SELECT MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(gap_days AS FLOAT(6)) END) AS median_days
    FROM (
        SELECT
            gap_days,
...
med AS (
    SELECT MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(days_before AS FLOAT(6)) END) AS median_days
    FROM (
        SELECT
            days_before,
...
med AS (
    SELECT MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(first_after_days AS FLOAT(6)) END) AS median_days
    FROM (
        SELECT
            first_after_days,
...
pairs AS (
    ...
    SELECT
        pd.person_id,
        pd.root_concept_id,
        (CAST(ld.l01_date AS DATE) - CAST(pd.procedure_date AS DATE)) AS gap_days
    FROM proc_dates pd
    JOIN l01_dates ld
...
WITH gdx_flags AS (
    ...
    SELECT
        g.person_id,
        MAX(CASE WHEN (CAST(g.event_date AS DATE) - CAST(c.index_date AS DATE)) <  0 THEN 1 ELSE 0 END) AS has_before,
        MAX(CASE WHEN (CAST(g.event_date AS DATE) - CAST(c.index_date AS DATE)) =  0 THEN 1 ELSE 0 END) AS has_day0,
        MAX(CASE WHEN (CAST(g.event_date AS DATE) - CAST(c.index_date AS DATE)) >  0 THEN 1 ELSE 0 END) AS has_after_strict
...
WITH first_general AS (
    ...
    SELECT
        gs.person_id,
        (CAST(gs.first_gen_cancer_date AS DATE) - CAST(c.index_date AS DATE)) AS signed_days
...
med AS (
    SELECT MIN(CASE WHEN 2.0 * rn >= cnt THEN CAST(signed_days AS FLOAT(6)) END) AS median_days
    FROM (
        SELECT
            signed_days,
