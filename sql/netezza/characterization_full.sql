...
SELECT 'OVERALL', (CAST(d.death_date AS DATE) - CAST(c.index_date AS DATE))
FROM cohort c
INNER JOIN (
...
UNION ALL
SELECT CAST(DATE_PART('YEAR', c.index_date) AS VARCHAR(4)), (CAST(d.death_date AS DATE) - CAST(c.index_date AS DATE))
FROM cohort c
INNER JOIN (
    SELECT person_id, MIN(death_date) AS death_date
    FROM @cdm_database_schema.death
...
SELECT 'OVERALL', (CAST(d.death_date AS DATE) - CAST(ms.first_met_date AS DATE))
FROM cohort c
...
UNION ALL
SELECT CAST(DATE_PART('YEAR', c.index_date) AS VARCHAR(4)), (CAST(d.death_date AS DATE) - CAST(ms.first_met_date AS DATE))
FROM cohort c
INNER JOIN met_summary ms ON c.person_id = ms.person_id AND ms.first_met_date IS NOT NULL
INNER JOIN (
    SELECT person_id, MIN(death_date) AS death_date
...
ORDER BY
    ...
    CAST(prevalence_year  AS INTEGER)
;
...
ORDER BY
    ...
    CAST(s.prevalence_year  AS INTEGER),
